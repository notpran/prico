...
export function TopNav({ user, currentView, onViewChange, onLogout }: TopNavProps) {
  const menuItems = [
        ...
        <div className="flex items-center space-x-4">
          ...
          >
            <div className="text-right">
              <p className="text-white text-sm font-inter">{user.name}</p>
              <p className="text-xs text-blue-300 capitalize font-mono">{user.status}</p>
            </div>
            <div className="relative">
              ...
              >
                <Circle 
                  className={`absolute -bottom-0.5 -right-0.5 h-3 w-3 ${getStatusColor(user.status)} fill-current`}
                />
              </motion.div>
            </div>
          </motion.div>
